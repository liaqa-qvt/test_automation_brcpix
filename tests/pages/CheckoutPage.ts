...
export class CheckoutPage {
  ...
  readonly switchConverterBRC: Locator;
  ...
  constructor(page: Page) {
    ...
    this.switchConverterBRC  = page.locator('button[role="switch"]').filter({
      has: page.locator('..').filter({ hasText: 'Converter pagamento em saldo BRC' })
    }).first();
    this.switchConverterUSDT = page.locator('button[role="switch"]').filter({
      has: page.locator('..').filter({ hasText: 'Converter pagamento em USDT' })
    }).first();
  ...
  async goto() {
    await this.page.goto('/painel/checkout');
  }
